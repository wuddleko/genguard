...
Later groups see the working tree as earlier groups left it, including files wiped by `clean`. Prefer disjoint `outputs` so a failed or drifting group cannot look like drift in the next one. After every group has run, genguard exits `1` if any group drifted or `2` if any group had a command error (command errors take priority).
